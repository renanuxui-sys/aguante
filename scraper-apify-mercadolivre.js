...
import { criarSupabase, salvarProdutos, extrairAno } from './scraper-utils.js'
...
function filtrarClubes(clubesSelecionados) {
  ...
  return CLUBES.filter(({ clube, termos = [] }) => {
...
async function main() {
  ...
  const clubes = filtrarClubes(options.clubesSelecionados)
