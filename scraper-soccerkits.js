/**
 * Scraper — Soccer Kits (Nuvemshop)
 * Roda com: node scraper-soccerkits.js
 * Teste sem salvar: node scraper-soccerkits.js --dry-run
 */
...
const BASE_URL = 'https://www.soccerkits.com.br'
const FONTE_NOME = 'Soccer Kits'
...
const COLECOES = [
  { path: '/clubes-brasileiros/', nome: 'Clubes brasileiros', categorias: ['Clubes Brasileiros'] },
  { path: '/camisas/', nome: 'Clubes internacionais', categorias: ['Clubes Europeus', 'Clubes Sulamericanos'], somenteIdentificados: true },
...
async function buscarHtml(url) {
  const res = await fetch(url, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (compatible; AguanteBot/1.0)',
...
async function main() {
  console.log(`🚀 Scraper — Soccer Kits${dryRun ? ' (dry-run)' : ''}\n`)
