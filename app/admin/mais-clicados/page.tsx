...
export default function MaisClicados() {
  ...
  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.03em', color: '#1A1A1A', margin: 0 }}>Mais clicados</h1>
        <p style={{ color: '#8A8880', fontSize: 14, marginTop: 4 }}>Ranking de cliques no botão "ir para o anúncio original"</p>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #E8E6DF' }}>
                {['#', 'Produto', 'Clube', 'Cliques anúncio', 'Views', 'Likes', ''].map(h => (
            ...
            <tbody>
              {produtos.map((p, i) => (
                <tr key={p.id} style={{ borderBottom: i < produtos.length - 1 ? '1px solid #F0EFEB' : 'none' }}>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: 14, fontWeight: 700, color: '#1A1A1A' }}>{(p.cliques_anuncio || 0).toLocaleString('pt-BR')}</td>
