...
const MOCK: Produto[] = [
  { id: '1', titulo: 'Camisa Flamengo Adidas', ano: '1994', preco: 2500, imagem_url: null, link_original: '#', fonte_nome: 'Mercado Livre', fonte_url: '', clube: 'Flamengo', tags: [], de_jogo: true, novidade: true, alta_procura: true, created_at: '', updated_at: '' },
  { id: '2', titulo: 'Camisa Internacional Perusso', ano: '1985', preco: 1800, imagem_url: null, link_original: '#', fonte_nome: 'OLX', fonte_url: '', clube: 'Internacional', tags: [], de_jogo: false, novidade: true, alta_procura: false, created_at: '', updated_at: '' },
  { id: '3', titulo: 'Camisa Santos Umbro', ano: '1998', preco: 900, imagem_url: null, link_original: '#', fonte_nome: 'Enjoei', fonte_url: '', clube: 'Santos', tags: [], de_jogo: false, novidade: false, alta_procura: true, created_at: '', updated_at: '' },
  { id: '4', titulo: 'Camisa Grêmio Topper', ano: '1996', preco: 1200, imagem_url: null, link_original: '#', fonte_nome: 'Mercado Livre', fonte_url: '', clube: 'Grêmio', tags: [], de_jogo: true, novidade: false, alta_procura: false, created_at: '', updated_at: '' },
  { id: '5', titulo: 'Camisa Corinthians Penalty', ano: '2000', preco: 750, imagem_url: null, link_original: '#', fonte_nome: 'OLX', fonte_url: '', clube: 'Corinthians', tags: [], de_jogo: false, novidade: true, alta_procura: false, created_at: '', updated_at: '' },
]
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase.from('produtos').select('*').order('created_at', { ascending: false }).limit(5)
      .then(({ data }) => setNovidades(data?.length ? data : MOCK))
    supabase.from('produtos').select('*').eq('alta_procura', true).limit(10)
      .then(({ data }) => setEmAlta(data?.length ? data : [...MOCK, ...MOCK].slice(0, 10)))
    supabase.from('produtos').select('*').limit(5)
      .then(({ data }) => setAnos80(data?.length ? data : MOCK))

    // Clubes com destaque para os atalhos da hero
            ...
            <div style={{ position: 'relative', width: 500, height: 545, flexShrink: 0, overflow: 'visible' }}>
              ...
              <div style={{ position: 'absolute', left: 56, top: 261, backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)', background: 'rgba(255,255,255,0.88)', borderTop: '1px solid white', borderRadius: 8, padding: 16, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '8px 12px 36px rgba(0,0,0,0.08)', zIndex: 10, width: 278 }}>
                ...
                <p style={{ fontSize: 14, color: 'rgba(0,0,0,0.6)', letterSpacing: '-0.01em', lineHeight: 1.2 }}>
                  <strong style={{ color: '#550fed', fontWeight: 700 }}>42 novos anúncios </strong>
              ...
              <div style={{ position: 'absolute', left: 222, top: 380, backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)', background: 'rgba(255,255,255,0.88)', borderTop: '1px solid white', borderRadius: 8, padding: 16, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '8px 12px 36px rgba(0,0,0,0.08)', zIndex: 10, width: 278 }}>
                ...
                <p style={{ fontSize: 14, color: 'rgba(0,0,0,0.6)', letterSpacing: '-0.01em', lineHeight: 1.2 }}>
                  <strong style={{ color: '#550fed', fontWeight: 700 }}>3.247 camisas </strong>
            ...
            <div className="ag-container">
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' as const, gap: 12 }}>
                {clubes.map(c => (
                  <button
                    key={c.id}
                    onClick={() => router.push(`/search?q=${c.nome}`)}
                    title={c.nome}
        ...
        <section style={{ background: '#f5f5f5', paddingBottom: 112 }}>
          <div className="ag-container">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              ...
              <button onClick={() => router.push('/search?filtro=alta_procura')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: '#62748c', fontFamily: 'Onest, sans-serif' }}>
                Ver todas →
              </button>
            </div>
