...
Exemplo com Originais do Fut:
```bash
node scraper-originaisdofut.js --dry-run
...
Exemplo com Mania de Camisa:
```bash
node scraper-maniadecamisa.js --dry-run
...
Exemplo com Soccer Kits:
```bash
node scraper-soccerkits.js --dry-run
...
Exemplo com Minha Camisa Vermelha:
```bash
node scraper-minha-camisa-vermelha.js --dry-run
node scraper-minha-camisa-vermelha.js
```
Página pública: `/lojas/minha-camisa-vermelha`
...
- **Lojas HTTP/API**: Brechó do Futebol, Jaiminho, Brechó FC, Manto Sagrado, Mundo da Bola, Copero Brechó, Di Gordo, Xiru FC, Rill Sports, Arara de Jogo, Originais do Fut, Camisa Doze, Old Collection 10, Soccer Kits, SR Futebol, Minha Camisa Vermelha e Virou Passeio Store em matriz paralela.
...
node scraper-minha-camisa-vermelha.js   # Minha Camisa Vermelha
...
### 14. Mania de Camisa (Bagy + Playwright)
**Site:** `maniadecamisafut.com.br`
**Arquivo:** `scraper-maniadecamisa.js`
**Abordagem:** Playwright + cheerio, porque a loja retorna 403 em requisições HTTP diretas
**Produtos:** a verificar
**Paginação automática:** ✅ para após 2 páginas vazias

**URLs rastreadas:** `/nacionais`, `/clubes-europeus`, `/sul-americanos`, `/selecoes`

**Comandos:**
```bash
node scraper-maniadecamisa.js --dry-run        # testa sem salvar no banco
node scraper-maniadecamisa.js                  # desativa antigos da fonte e salva ativos
node scraper-maniadecamisa.js --sem-desativar  # salva sem desativar antigos
```
...
### 16. Soccer Kits (Nuvemshop)
**Site:** `soccerkits.com.br`
**Arquivo:** `scraper-soccerkits.js`
**Abordagem:** node-fetch + cheerio
**Produtos:** a verificar
**Paginação automática:** ✅ para após 2 páginas vazias

**URLs rastreadas:** `/clubes-brasileiros/`, `/camisas/`, `/selecoes/`

**Comandos:**
```bash
node scraper-soccerkits.js --dry-run        # testa sem salvar no banco
node scraper-soccerkits.js                  # desativa antigos da fonte e salva ativos
node scraper-soccerkits.js --sem-desativar  # salva sem desativar antigos
```
